...
if (isset($_POST['status-bijwerken'])) {
	...
	if ($validateRegister->validateInfo($data)) {
		...
		$columns = array(
			'carColorName' => input::get('statusName'));

		$extraOptions = array("carColorId" => $_GET['edit']);
...
if (isset($_GET['edit'])) {
	$dataUser = null;
	if ($car->getCarz($table, $_GET['edit'])) {

		$dataUser = $car->getCarz($table, $_GET['edit'])[0];
	}

	if ($dataUser) {
		$idValid = true;
		$_POST['txt_status'] = $dataUser['carColorName'];
...
if (isset($_GET['delete'])) {
	if ( $car->getCarz($table, $_GET['delete'] ) ) {
		$idValidDelete = true;
	}
	if ( isset( $_POST['delete-submit'] ) && $idValidDelete ) {
		$extraOptions = array( 'carColorId' => $_GET['delete'] );
		$updated = true;
				...
				<table class="table table-striped">
					...
					<tr>
						...
						foreach ( $car->getAllCars( $table ) as $singleRowData ) {
							...
							echo "<td>
                                        <a class='' href='?edit=" . $singleRowData['carColorId'] . "'>
                                            <button class='btn btn-default glyphicon glyphicon-pencil'></button>
                                        </a>
                                           <a class='' href='?delete=" . $singleRowData['carColorId'] . "'>
