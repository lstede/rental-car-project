...
if ( isset( $_POST['status-toevoegen'] ) ) {
	...
	$columns = array(
		'carBrandsName' => $_POST['txt_status']
	);
...
if (isset($_POST['status-bijwerken'])) {
	...
	if ($validateRegister->validateInfo($data)) {
		...
		$columns = array(
			'carBrandsName' => input::get('statusName'));

		$extraOptions = array("carBrandsId" => $_GET['edit']);
...
if (isset($_GET['edit'])) {
	$dataUser = null;
	if ($car->getCarBrands($table, $_GET['edit'])) {

		$dataUser = $car->getCarBrands($table, $_GET['edit'])[0];
	}

	if ($dataUser) {
		$idValid = true;
		$_POST['txt_status'] = $dataUser['carBrandsName'];
...
if (isset($_GET['delete'])) {
	if ( $car->getCarBrands($table, $_GET['delete'] ) ) {
		$idValidDelete = true;
	}
	if ( isset( $_POST['delete-submit'] ) && $idValidDelete ) {
		$extraOptions = array( 'carBrandsId' => $_GET['delete'] );
		$updated = true;
				...
				<table class="table table-striped">
					...
					<tr>
						...
						foreach ( $car->getCars( $table ) as $singleRowData ) {
							...
							echo "<td>
                                        <a class='' href='?edit=" . $singleRowData['carBrandsId'] . "'>
                                            <button class='btn btn-default glyphicon glyphicon-pencil'></button>
                                        </a>
                                           <a class='' href='?delete=" . $singleRowData['carBrandsId'] . "'>
